...
> **说明：** 本文档从 MES Demo 迁入，API 路径（如 `/api/agent/chat`）、Session 表名（`agent_sessions`）等为 **MES 示例**。  
> **gaosi-tutor 对照：** 流式 `POST /api/chat/stream`、Session 表 `tutor_sessions` / `tutor_messages`、无 `agent_traces` / `agent_eval.py`。概念（SSE、Session、Eval）仍适用。

> 本文档说明：阶段 5 如何把现有 Agent 从「能跑」升级到「能演示、能调试、能回归」。  
> 四大能力：**SSE 流式**、**对话记忆**、**可观测日志**、**评估用例**。  
> 前置：Tool Calling + RAG（gaosi-tutor：`search_family_notes`）已就绪；MES 另有写操作确认门。
...
- 「为什么没调 search_family_notes？」→ 对比 question 和 tool_trace（gaosi-tutor RAG tool）
...
```python
# MES 示例（gaosi-tutor 可类比为家庭笔记 / 出题类用例）
CASES = [
    {
        "id": "lesson_context",
        "question": "第 5 讲是什么内容？",
        "expect_tools": ["get_lesson_context"],
    },
    {
        "id": "family_notes_rag",
        "question": "根据家庭笔记，孩子减法哪里薄弱？",
        "expect_tools": ["search_family_notes"],
        "answer_contains": ["减"],
    },
    {
        "id": "generate_practice",
        "question": "出一道题",
        "expect_tools": ["generate_practice"],
    },
    # MES 原例：get_work_order / create_work_order / search_sop …
]
```
...
[FAIL] family_notes_rag    expected search_family_notes, got []  3.4s
---
8/10 passed
```
